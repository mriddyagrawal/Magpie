...
| `defaults` | Hardcoded ignores covering: VCS dirs (`.git/`, `.hg/`, `.svn/`); language build artifacts (Python `__pycache__/`, JS `node_modules/` + `.next/` + `.parcel-cache/` + `.turbo/` + `.vercel/` + `.netlify/` + `.svelte-kit/` + `.angular/` + `.firebase/` + `bower_components/`; Rust `target/` + `*.rs.bk`; .NET `bin/`/`obj/` + `*.suo`/`*.user`/`*.tlog`; CMake/autotools `CMakeFiles/` + `.deps/` + `.libs/` + `autom4te.cache/` + `config.{log,status,cache}` + `*.dSYM/`; JVM `.gradle/` + `*.iml`/`*.ipr`/`*.iws`; Swift/Xcode `DerivedData/` + `xcuserdata/` + `Pods/` + `.swiftpm/`; Haskell `.stack-work/` + `dist-newstyle/` + `.cabal-sandbox/`; Erlang/Elixir `_build/` + `.elixir_ls/`; Scala `.bloop/` + `.metals/` + `.bsp/`; Ruby `.bundle/` + `vendor/bundle/`); generic build dirs (`build/` `dist/` `out/` `coverage/`); OS cruft (Windows `$RECYCLE.BIN/` + `System Volume Information/` + `WindowsApps/` + `Program Files/` + `ProgramData/` + `$WINDOWS.~BT/` + `pagefile.sys` + `NTUSER.DAT*` + `~$*` Office locks; macOS `.DS_Store` + `.fseventsd/` + `.Spotlight-V100/` + `.AppleDouble/` + `._*` + `Library/{Containers,Logs,CrashReporter}/` + `*.sparsebundle/` + `Backups.backupdb/`; Linux `lost+found/` + `.snapshots/` + `.zfs/snapshot/` + `core` (literal) + `*~` + `\#*#` + `.#*`); system temps + caches (`tmp/` `var/tmp/` `.cache/` `Library/Caches/`); lockfiles (`package-lock.json` etc.); OOXML decomposition (`*_extracted/{ppt,word,xl}/media/`); and **common-secret filenames** (`.env*`, `.npmrc`, `.netrc`, `.pgpass`, `.git-credentials`, `id_rsa[.*]`, `id_ed25519[.*]`, `id_ecdsa[.*]`, `id_dsa[.*]`, `*.pfx`, `*.p12`). Cannot be overridden by user config. Deliberately NOT in defaults: `vendor/` (too generic), `*.log` (legitimate user content), `core.*` (collides with `core.py`), `packages/`/`deps/` (collide with user folders). |
| `dataopt` | `.json`, `.csv`, `.dat` are **default-skipped** — real-world corpora carry hundreds (config dumps, dataset exports) that bloat the index without typically being what users query. `--include-data` re-adds them per-run. `.yaml`/`.yml`/`.toml` are NOT in this set (small config files like `pyproject.toml` are usually wanted). |
...
| `include-data` | `python -m src.ingest <dir> --include-data` — re-add `.json`, `.csv`, `.dat` to the candidate set (they're default-skipped because most user corpora have hundreds of these as auto-generated config / data dumps that bloat the index). Use when you want to search across structured datasets. |
