...
- Reads `Test Summaries/*.md` → `ParsedSummary` dataclass
...
├── Test Summaries/                         (generated by Stage 1)
...
1. `python -m notanotherspotlight ingest` ingests all summaries from `Test Summaries/` into Qdrant Cloud without errors
