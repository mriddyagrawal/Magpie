...
| Default profile picks per-call (text → vision swap on image; vision → text swap on text-only). Cost: ~25-30s per swap with `MAX_LOADED_MODELS=1`. | **Default singleton is bound to `gemma-4-e4b-vision` for ALL inference.** The text-only profile (`gemma-4-e4b-text`) is still registered as an opt-in memory-saver. | Found by user during the first real `just sync`: a mixed corpus thrashed the LRU pool. The fix is conceptually trivial — Gemma 4 + mmproj is one set of weights with an optional vision encoder. When mmproj is loaded but no `image_url` block is in the request, the projector tensors aren't part of the forward pass — zero inference cost, ~946 MB resident-memory cost. Verified empirically (text-only "What is 2+2?", image-bearing "describe", text-only "Capital of Japan" — all three on one subprocess, single PID, no swaps). Architecture is universal across mmproj-using models: Qwen2.5-VL, LFM2-VL, MiniCPM-V all behave the same way per llama.cpp's [multimodal docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/multimodal.md), so the pattern carries forward when we add those profiles. Users on tight RAM budgets opt out via `LLAMA_SERVER_TEXT_MODEL=gemma-4-e4b-text` in `.env`. |
