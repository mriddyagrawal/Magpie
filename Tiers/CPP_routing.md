...
These extensions take the **same** code path: `.py`, `.js`, `.ts`, `.tsx`, `.jsx`, `.go`, `.rs`, `.java`, `.c`, `.h`, `.hpp`, `.cs`, `.rb`, `.swift`, `.kt`, `.sh`, `.sql`.

## Flowchart

```mermaid
flowchart TD
    A["File: example.cpp"] --> B{"In .gitignore /<br/>.nasignore /<br/>default-ignore?"}
