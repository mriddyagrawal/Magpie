...
These extensions take the **same** code path: `.js`, `.ts`, `.tsx`, `.jsx`, `.go`, `.rs`, `.java`, `.c`, `.cpp`, `.h`, `.hpp`, `.cs`, `.rb`, `.swift`, `.kt`, `.sh`, `.sql`.

## Flowchart

```mermaid
flowchart TD
    A["File: example.py"] --> B{"In .gitignore /<br/>.nasignore /<br/>default-ignore?"}
