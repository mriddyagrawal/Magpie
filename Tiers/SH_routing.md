...
These extensions take the **same** code path: `.py`, `.js`, `.ts`, `.tsx`, `.jsx`, `.go`, `.rs`, `.java`, `.c`, `.cpp`, `.h`, `.hpp`, `.cs`, `.rb`, `.swift`, `.kt`, `.sql`.

## Flowchart

```mermaid
flowchart TD
    A["File: example.sh"] --> B{"In .gitignore /<br/>.nasignore /<br/>default-ignore?"}
