...
These extensions take the **same** code path: `.py`, `.js`, `.ts`, `.tsx`, `.jsx`, `.go`, `.rs`, `.java`, `.c`, `.cpp`, `.h`, `.hpp`, `.cs`, `.rb`, `.swift`, `.kt`, `.sh`.

## Flowchart

```mermaid
flowchart TD
    A["File: example.sql"] --> B{"In .gitignore /<br/>.nasignore /<br/>default-ignore?"}
