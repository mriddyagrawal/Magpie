...
| then answers | Context budget → optional extractive span → prompt assembly → KV prefix restore or build → local LFM2.5-VL-3B under a GBNF grammar, or a cloud model with JSON repair → not-found contract → numeral grounding guard → source path resolution. The reply is one JSON object; on `/query/stream` its `answer` value is shown to the user as the model writes it (`answer_stream.py` decodes it out of the token stream), and the checked answer follows as `answer_final`. | `answer.answer_question`, `answer_stream.py`, `kv_cache.py`, `inference/gbnf.py`, `grounding.py` |
