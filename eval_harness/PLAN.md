...
| `rerank` (`MAGPIE_RERANK`) | on (prod default) vs off. Off = results come back in RRF-fusion order (for visual hits, ColQwen's own ranking) instead of cross-encoder order | retrieval + answer | full run per value. **Two-factor change**: off also structurally disables `solo_gate` (its margin threshold is on cross-encoder score scale) — a "rerank ablation" at rerank=off is really rerank+gate; `solo_gate_structurally_off` is stamped in run.json so the confound travels with the data |
