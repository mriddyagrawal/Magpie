...
def _extract_book_page_label(text: str) -> str | None:
    ...
    Many PDFs (especially scans / books typeset before PDF page-label
    metadata existed) carry the printed page number as a small standalone
    number in the running header or footer. The PDF page index (1, 2, 3…)
    almost never matches the book's printed page (which restarts at 1
    after front matter and may use roman numerals before that).

    Heuristic: look at the first 3 lines of the page (most academic books
    use top-of-page page numbers — Taylor's textbook is one of these), then
    the last 3 lines (footer style). A line is a page-label if it contains
    only a 1-4 digit number or a Roman numeral, surrounded by whitespace.

    Returns the label string ("254" / "vi") or None if no clean label was
    detectable. Used purely for human-facing citation; the algorithm never
    relies on the recovered value.
    """
...
def _looks_like_toc_page(text: str) -> bool:
    ...
    TOC pages list every chapter and section by name → they always contain
    half a dozen or more keywords from any "what's in this textbook" query
    → without filtering they outscore the actual content pages where each
    term appears once or twice in prose. The bookmark TOC is already in the
    summary tier (via `_extract_pdf_toc`), so excluding TOC pages from the
    lazy candidate set costs nothing and avoids feeding the LLM the same TOC
    twice.

    Threshold of 5 picked empirically: real content pages occasionally have
    1-2 numbered subsection headings; TOC pages have 10-30. Tested on the
    Taylor textbook where this catches pages 6-15 (the TOC) without false-
    positiving on chapter-opening pages that have 1-2 section headers.
    """
...
def extract_pdf_relevant_pages(
    ...
    *,
    ...
    The lazy-chunking primitive. Used at **answer time** for long PDFs that
    would otherwise have their first ~25 K chars (≈ 10 pages of cover +
    preface) cut and sent to the LLM, missing the actual content the user
    asked about further into the file.

    Algorithm:
      1. For each page, count case-insensitive substring hits across all
         provided keywords.
      2. Pick the top `max_pages` pages by score.
      3. Expand to include `±context_pages` neighbors of each match (so a
         section heading on page 247 gives the LLM page 246-248, not just 247).
      4. Emit pages in document order with "## Page N" headers, capping the
         total at `max_chars`.

    Returns "" when:
      - `keywords` is empty (caller should fall back to `extract_pdf_text`)
      - no page contains any keyword (fall back to first-N-chars)
      - PDF can't be opened (caller decides whether to raise or fall back)

    NOTE: this is a within-file ripgrep-shaped retrieval — not the same as
    upfront chunking (B8). Files where the relevant term isn't surface-
    visible (no TOC, no abstract mention) won't be retrieved at the index
    level, so this function never gets a chance to lazy-chunk them. That's
    the deliberate scope; conditional upfront chunking is deferred.
    """
    if not keywords:
        return ""
    ...
    try:
        ...
        # Score each page by total keyword hit count. Case-insensitive,
        # substring match — same idiom as ripgrep `-i`.
        ...
        for i in range(n_pages):
            try:
                ...
                continue
            # Skip TOC pages — they list every chapter heading so they always
            # outscore real content pages on any "what's in this book" query,
            # but they don't contain the actual content the user asked about.
            # The bookmark TOC is already in the summary tier; we don't need
            # it doubled up in the lazy chunks too.
        ...
        # Top-K by score, then expand with neighbor pages for context. We
        # union both sets so a single page that scored 17 hits doesn't blow
        # the budget on its 2 neighbors at the expense of a different page
        # that also scored a real hit.
        ...
        # Emit in document order, with page anchors so the LLM can cite both
        # the PDF page (digital, what you see in a PDF reader) and the book's
        # printed page label (what's at the top/bottom of the page in the
        # actual book). The LLM is instructed in the system prompt to use
        # both forms when citing — see src/answer.py:SYSTEM_PROMPT.
        chunks: list[str] = []
        total = 0
